...
export async function waitForLiveSession(page, context = "session") {
  ...
  try {
    // First try to wait for canvas (indicates video stream)
    ...
    try {
      // Check for live session UI elements
      await page.waitForSelector('[aria-label="Microphone"], [aria-label="Camera"], .live-controls, [data-testid*="live"]', { timeout: 10000 });
      console.log(`✅ ${context}: Live session controls detected`);
      return true;
    } catch (error2) {
      console.log(`⚠️ ${context}: No live controls found, waiting for page to stabilize...`);
      // Final fallback - just wait for the page to stabilize
      ...
      console.log(`⚠️ ${context}: Proceeding without video confirmation`);
      return false;
    }
  }
}
